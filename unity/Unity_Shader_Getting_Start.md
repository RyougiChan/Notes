...
把平移、旋转和缩放组合起来，所形成的一个复杂的变换过程。

在绝大多数情况下，我们**约定**变换的顺序就是**先缩放，再旋转，最后平移**。
除了需要注意不同类型的变换顺序外，还需要小心旋转的变换顺序。当我们直接给出 `(θx,θy, θz)` 这样的旋转角度时，需要定义一个旋转顺序。在 Unity 中，这个旋转顺序是`zxy`。
